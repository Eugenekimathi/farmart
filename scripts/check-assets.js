...
const files = walk(path.join(frontend, 'src'), exts).concat(walk(path.join(frontend, 'public'), exts)).concat([path.join(frontend, 'index.html')].filter(fs.existsSync));
...
for (const file of files) {
  ...
  while ((m = assetPattern.exec(txt)) !== null) {
    ...
    let candidate;
    if (raw.startsWith('/')) {
      // public root: check both Frontend/<file> and Frontend/public/<file>
      const without = raw.replace(/^\//, '');
      const candidatePublic = path.join(frontend, 'public', without);
      const candidateRoot = path.join(frontend, without);
      ...
      candidate = path.join(frontend, raw);
    } else {
      // try relative to file
      candidate = path.resolve(path.dirname(file), raw);
      if (!fs.existsSync(candidate)) candidate = path.join(frontend, raw);
    }
